...
namespace winapic {
	...
	float acosf(float _X) { return _mm_cvtss_f32(_mm_acos_ps(_mm_set_ss(_X))); }
	float powf(float _X, float _Y) { return _mm_cvtss_f32(_mm_pow_ps(_mm_set_ss(_X), _mm_set_ss(_Y))); }
	float atan2f(float _X, float _Y) { return _mm_cvtss_f32(_mm_atan2_ps(_mm_set_ss(_X), _mm_set_ss(_Y))); }
}
...
ImGuiIO::ImGuiIO()
{
	...
	winapic::memset(this, 0, sizeof(*this));
...
ImVec2 ImLineClosestPoint(const ImVec2& a, const ImVec2& b, const ImVec2& p)
{
	ImVec2 ap = p - a;
	ImVec2 ab_dir = b - a;
	float ab_len = winapic::sqrtf(ab_dir.x * ab_dir.x + ab_dir.y * ab_dir.y);
	ab_dir *= 1.0f / ab_len;
...
{
	int d;
	while ((d = winapic::toupper(*str2) - winapic::toupper(*str1)) == 0 && *str1) { str1++; str2++; }
	return d;
}
...
{
	int d = 0;
	while (count > 0 && (d = winapic::toupper(*str2) - winapic::toupper(*str1)) == 0 && *str1) { str1++; str2++; count--; }
	return d;
}
...
{
	size_t len = winapic::strlen(str) + 1;
	void* buf = ImGui::MemAlloc(len);
	return (char*)winapic::memcpy(buf, (const void*)str, len);
}
...
{
	if (!needle_end)
		needle_end = needle + winapic::strlen(needle);

	const char un0 = (char)winapic::toupper(*needle);
	while ((!haystack_end && *haystack) || (haystack_end && haystack < haystack_end))
	{
		if (winapic::toupper(*haystack) == un0)
		{
			const char* b = needle + 1;
			for (const char* a = haystack + 1; b < needle_end; a++, b++)
				if (winapic::toupper(*a) != winapic::toupper(*b))
					break;
			if (b == needle_end)
				return haystack;
		}
...
int ImFormatString(char* buf, size_t buf_size, const char* fmt, ...)
{
	//va_list args;
	//va_start(args, fmt);
	//int w = safe_call(vsnprintf)(buf, buf_size, fmt, args);
	//va_end(args);
	//if (buf == NULL)
		//return w;
...
void ImGui::ColorConvertRGBtoHSV(float r, float g, float b, float& out_h, float& out_s, float& out_v)
{
	...
	out_h = winapic::fabsf(K + (g - b) / (6.f * chroma + 1e-20f));
...
void ImGui::ColorConvertHSVtoRGB(float h, float s, float v, float& out_r, float& out_g, float& out_b)
{
	...
	//h = safe_call(fmodf)(h, 1.0f) / (60.0f / 360.0f);
	int   i = (int)h;
	float f = h - (float)i;
	float p = v * (1.0f - s);
	float q = v * (1.0f - s * f);
...
void* ImFileLoadToMemory(const char* filename, const char* file_open_mode, int* out_file_size, int padding_bytes)
{
	...
	if (padding_bytes > 0)
		winapic::memset((void*)(((char*)file_data) + file_size), 0, padding_bytes);
...
{
	Filters.resize(0);
	TextRange input_range(InputBuf, InputBuf + winapic::strlen(InputBuf));
...
ImGuiMenuColumns::ImGuiMenuColumns()
{
	...
	winapic::memset(Pos, 0, sizeof(Pos));
	winapic::memset(NextWidths, 0, sizeof(NextWidths));
}
...
void ImGuiMenuColumns::Update(int count, float spacing, bool clear)
{
	...
	Spacing = spacing;
	if (clear) winapic::memset(NextWidths, 0, sizeof(NextWidths));
...
void* ImGui::memmove_(void* dest, const void* src, unsigned __int64 count)
{
	char* char_dest = (char*)dest;
	char* char_src = (char*)src;
	if ((char_dest <= char_src) || (char_dest >= (char_src + count)))
...
void* ImGui::memcpy_(void* dest, const void* src, unsigned __int64 count)
{
	char* char_dest = (char*)dest;
	char* char_src = (char*)src;
	if ((char_dest <= char_src) || (char_dest >= (char_src + count)))
...
void* ImGui::memset_(void* dst0, register int c0, register size_t length)
{
	register size_t t;
	register u_char* dst;
...
int ImGui::strlen_(const char* string)
{
	int cnt = 0;
	if (string)
	{
...
int ImGui::memcmp_(const void* s1, const void* s2, unsigned __int64 n)
{
	if (n != 0)
	{
		const unsigned char* p1 = (unsigned char*)s1, * p2 = (unsigned char*)s2;
...
void ImGui::NewFrame()
{
	...
	if (g.DragDropActive && g.DragDropPayload.DataFrameCount + 1 < g.FrameCount)
	{
		...
		winapic::memset(&g.DragDropPayloadBufLocal, 0, sizeof(g.DragDropPayloadBufLocal));
	}
	...
	winapic::memcpy(g.IO.KeysDownDurationPrev, g.IO.KeysDownDuration, sizeof(g.IO.KeysDownDuration));
...
static void SettingsHandlerWindow_WriteAll(ImGuiContext* imgui_ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf)
{
	...
	for (int i = 0; i != g.SettingsWindows.Size; i++)
	{
		...
		if (const char* p = winapic::strstr(name, _("###")))  // Skip to the "###" marker if any. We don't skip past to match the behavior of GetID()
			name = p;
...
static void LoadIniSettingsFromMemory(const char* buf_readonly)
{
	...
	char* buf_end = buf + winapic::strlen(buf);
...
void ImDrawDataBuilder::FlattenIntoSingleLayer()
{
	...
	for (int layer_n = 1; layer_n < IM_ARRAYSIZE(Layers); layer_n++)
	{
		...
		if (layer.empty())
			continue;
		winapic::memcpy(&Layers[0][n], &layer[0], layer.Size * sizeof(ImDrawList*));
		n += layer.Size;
		layer.resize(0);
	}
}
...
void ImGui::EndFrame()
{
	...
	winapic::memset(g.IO.InputCharacters, 0, sizeof(g.IO.InputCharacters));
	...
	for (;;)
	{
		...
		if (line_end != NULL && !(is_last_line && (line_end - text_remaining) == 0))
		{
			...
			//if (log_new_line || !is_first_line)
				//ImGui::LogText(IM_NEWLINE _("%*s%.*s"), tree_depth * 4, _(""), char_count, text_remaining);
			//else
				//ImGui::LogText(_(" %.*s"), char_count, text_remaining);
		}
...
void ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash)
{
	...
	{
		if (!text_end)
			text_end = text + winapic::strlen(text); // FIXME-OPT
...
bool ImGui::IsMouseClicked(int button, bool repeat)
{
	...
	if (repeat && t > g.IO.KeyRepeatDelay)
	{
		...
		//if ((safe_call(fmodf)(t - delay, rate) > rate * 0.5f) != (safe_call(fmodf)(t - delay - g.IO.DeltaTime, rate) > rate * 0.5f))
			//return true;
	}
...
void ImGui::BringWindowToBack(ImGuiWindow* window)
{
	...
	for (int i = 0; i < g.Windows.Size; i++)
		if (g.Windows[i] == window)
		{
			winapic::memmove(&g.Windows[1], &g.Windows[0], (size_t)i * sizeof(ImGuiWindow*));
			g.Windows[0] = window;
			break;
		}
}
		...
		if (text_pos.y <= clip_rect.Max.y)
		{
			...
			if (!g.LogEnabled)
			{
				...
				if (lines_skippable > 0)
				{
					...
					{
						const char* line_end = winapic::strchr(line, '\n');
						if (!line_end)
							line_end = text_end;
						line = line_end + 1;
						lines_skipped++;
...
static bool DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* data_ptr, const char* scalar_format)
{
	...
	if (data_type == ImGuiDataType_Int)
	{
		...
		float arg1f = 0.0f;
		//if (op == '+') { if (sscanf(buf, _("%f"), &arg1f) == 1) *v = (int)(arg0i + arg1f); }                 // Add (use "+-" to subtract)
...
int ImGui::ParseFormatPrecision(const char* fmt, int default_precision)
{
	...
	while ((fmt = winapic::strchr(fmt, '%')) != NULL)
	{
		fmt++;
...
static float GetMinimumStepAtDecimalPrecision(int decimal_precision)
{
	...
	return (decimal_precision >= 0 && decimal_precision < 10) ? min_steps[decimal_precision] : winapic::powf(10.0f, (float)-decimal_precision);
}
...
float ImGui::RoundScalar(float value, int decimal_precision)
{
	...
	value = winapic::fabsf(value);
	//float remainder = safe_call(fmodf)(value, min_step);
...
static inline float SliderBehaviorCalcRatioFromValue(float v, float v_min, float v_max, float power, float linear_zero_pos)
{
	...
	if (is_non_linear)
	{
		if (v_clamped < 0.0f)
		{
			...
			return (1.0f - winapic::powf(f, 1.0f / power)) * linear_zero_pos;
		}
		else
		{
			const float f = (v_clamped - ImMax(0.0f, v_min)) / (v_max - ImMax(0.0f, v_min));
			return linear_zero_pos + winapic::powf(f, 1.0f / power) * (1.0f - linear_zero_pos);
		}
	}
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags)
{
	...
	{
		// Different sign
		const float linear_dist_min_to_0 = winapic::powf(winapic::fabsf(0.0f - v_min), 1.0f / power);
		const float linear_dist_max_to_0 = winapic::powf(winapic::fabsf(v_max - 0.0f), 1.0f / power);
		...
		if (set_new_value)
		{
			...
			if (is_non_linear)
			{
				...
				else
				{
					...
					float a;
					if (winapic::fabsf(linear_zero_pos - 1.0f) > 1.e-6f)
...
bool ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
	...
	if (g.ActiveId == id)
	{
		if (g.IO.MouseDown[0])
		{
			...
			if (winapic::fabsf(adjust_delta) > 0.0f)
			{
				if (winapic::fabsf(power - 1.0f) > 0.001f)
				{
					// Logarithmic curve on both side of 0.0
					float v0_abs = v_cur >= 0.0f ? v_cur : -v_cur;
					float v0_sign = v_cur >= 0.0f ? 1.0f : -1.0f;
					float v1 = winapic::powf(v0_abs, 1.0f / power) + (adjust_delta * v0_sign);
					float v1_abs = v1 >= 0.0f ? v1 : -v1;
					float v1_sign = v1 >= 0.0f ? 1.0f : -1.0f;          // Crossed sign line
					v_cur = winapic::powf(v1_abs, power) * v0_sign * v1_sign;    // Reapply sign
				}
				else
				{
					v_cur += adjust_delta;
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static bool STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int pos, const ImWchar* new_text, int new_text_len)
	{
		...
		if (pos != text_len)
			winapic::memmove(text + pos + new_text_len, text + pos, (size_t)(text_len - pos) * sizeof(ImWchar));
		winapic::memcpy(text + pos, new_text, (size_t)new_text_len * sizeof(ImWchar));
...
{
	const int new_text_len = new_text_end ? (int)(new_text_end - new_text) : (int)winapic::strlen(new_text);
	if (new_text_len + BufTextLen + 1 >= BufSize)
		return;

	if (BufTextLen != pos)
		winapic::memmove(Buf + pos + new_text_len, Buf + pos, (size_t)(BufTextLen - pos));
	winapic::memcpy(Buf + pos, new_text, (size_t)new_text_len * sizeof(char));
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (flags & ImGuiInputTextFlags_CallbackCharFilter)
	{
		...
		winapic::memset(&callback_data, 0, sizeof(ImGuiTextEditCallbackData));
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		else if (is_paste)
		{
			...
			if (const char* clipboard = GetClipboardText())
			{
				...
				const int clipboard_len = (int)winapic::strlen(clipboard);
		...
		if (apply_edit_back_to_user_buffer)
		{
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					{
						IM_ASSERT(callback_data.BufTextLen == (int)winapic::strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
			...
			if (is_editable && winapic::strcmp(edit_state.TempTextBuffer.Data, buf) != 0)
			{
	...
	if (g.ActiveId == id || is_currently_scrolling)
	{
		...
		//bool cursor_is_visible = (!g.IO.OptCursorBlink) || (g.InputTextState.CursorAnim <= 0.0f) || safe_call(fmodf)(g.InputTextState.CursorAnim, 1.20f) <= 0.80f;
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
	...
	if (InputText(_(""), buf, IM_ARRAYSIZE(buf), extra_flags)) // PushId(label) + "" gives us the expected ID from outside point of view
...
bool ImGui::InputFloat(const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags extra_flags)
{
	...
	if (decimal_precision < 0)
		winapic::strcpy(display_format, _("%f"));      // Ideally we'd have a minimum decimal precision of 1 to visually denote that this is a float, while hiding non-significant digits? %f doesn't have a minimum of 1
	else
	...
	if (window->DC.LayoutType != ImGuiLayoutType_Horizontal) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
	{
		...
		if (g.HoveredWindow == window && g.OpenPopupStack.Size > g.CurrentPopupStack.Size && g.OpenPopupStack[g.CurrentPopupStack.Size].ParentWindow == window)
		{
			if (ImGuiWindow* next_window = g.OpenPopupStack[g.CurrentPopupStack.Size].Window)
			{
				...
				float extra = ImClamp(winapic::fabsf(ta.x - tb.x) * 0.30f, 5.0f, 30.0f); // add a bit of extra slack.
...
static void ColorPickerOptionsPopup(ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			winapic::memcpy(&dummy_ref_col.x, ref_col, sizeof(float) * (picker_flags & ImGuiColorEditFlags_NoAlpha ? 3 : 4));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if (window->DC.LastItemRectHoveredRect && BeginDragDropTarget()) // NB: The LastItemRectHoveredRect test is merely an optional micro-optimization
	{
		...
		{
			winapic::memcpy((float*)col, payload->Data, sizeof(float) * 3);
			value_changed = true;
		}
		if (const ImGuiPayload* payload = AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F))
		{
			winapic::memcpy((float*)col, payload->Data, sizeof(float) * components);
			value_changed = true;
		}
		EndDragDropTarget();
	}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	winapic::memcpy(backup_initial_col, col, components * sizeof(float));
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		if (IsItemActive())
		{
			...
			if (initial_dist2 >= (wheel_r_inner - 1) * (wheel_r_inner - 1) && initial_dist2 <= (wheel_r_outer + 1) * (wheel_r_outer + 1))
			{
				...
				H = winapic::atan2f(current_off.y, current_off.x) / IM_PI * 0.5f;
				if (H < 0.0f)
					H += 1.0f;
				value_changed = value_changed_h = true;
			}
			float cos_hue_angle = winapic::cosf(-H * 2.0f * IM_PI);
			float sin_hue_angle = winapic::sinf(-H * 2.0f * IM_PI);
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if (ref_col != NULL)
		{
			...
			{
				winapic::memcpy(col, ref_col, components * sizeof(float));
				value_changed = true;
			}
		}
		EndGroup();
	...
	if (flags & ImGuiColorEditFlags_PickerHueWheel)
	{
		...
		for (int n = 0; n < 6; n++)
		{
			...
			ImVec2 gradient_p0(wheel_center.x + winapic::cosf(a0) * wheel_r_inner, wheel_center.y + winapic::sinf(a0) * wheel_r_inner);
			ImVec2 gradient_p1(wheel_center.x + winapic::cosf(a1) * wheel_r_inner, wheel_center.y + winapic::sinf(a1) * wheel_r_inner);
	...
	return value_changed && winapic::memcmp(backup_initial_col, col, components * sizeof(float));
}
...
void ImGui::Separator(float heightdistance)
{
	...
	//if (g.LogEnabled)
		//LogRenderedText(NULL, IM_NEWLINE _("--------------------------------")));
...
bool ImGui::SetDragDropPayload(const char* type, const void* data, size_t data_size, ImGuiCond cond)
{
	...
	IM_ASSERT(type != NULL);
	IM_ASSERT(custom::strlen(type) < IM_ARRAYSIZE(payload.DataType) && _("Payload type can be at most 12 characters long"));
	...
	if (cond == ImGuiCond_Always || payload.DataFrameCount == -1)
	{
		...
		if (data_size > sizeof(g.DragDropPayloadBufLocal))
		{
			...
			winapic::memcpy((void*)payload.Data, data, data_size);
		}
		else if (data_size > 0)
		{
			// Store locally
...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
	...
	buf_local.clear();
	if (!safe_call(OpenClipboard)(NULL))
		return NULL;
	HANDLE wbuf_handle = safe_call(GetClipboardData)(CF_UNICODETEXT);
	if (wbuf_handle == NULL)
	{
		safe_call(CloseClipboard)();
		return NULL;
	}
...
//	}
	...
	//GlobalUnlock(wbuf_handle);
	//safe_call(EmptyClipboard)();
	//safe_call(SetClipboardData)(CF_UNICODETEXT, wbuf_handle);
	//safe_call(CloseClipboard)();
}
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
	ImGuiContext& g = *GImGui;
	g.PrivateClipboard.clear();
	const char* text_end = text + winapic::strlen(text);
	g.PrivateClipboard.resize((int)(text_end - text) + 1);
	winapic::memcpy(&g.PrivateClipboard[0], text, (size_t)(text_end - text));
...
//	if (HWND hwnd = (HWND)GImGui->IO.ImeWindowHandle)
	//	if (HIMC himc = safe_call(ImmGetContext)(hwnd))
	//	{
		//	COMPOSITIONFORM cf;
			//cf.ptCurrentPos.x = x;
			//cf.ptCurrentPos.y = y;
			//cf.dwStyle = CFS_FORCE_POSITION;
			//safe_call(ImmSetCompositionWindow)(himc, &cf);
		//}
}
...
void ImGui::ShowMetricsWindow(bool* p_open)
{
	if (ImGui::Begin(_("ImGui Metrics"), p_open))
	{
		...
		struct Funcs
		{
			...
			{
				bool node_open = ImGui::TreeNode(draw_list, _("%s: '%s' %d vtx, %d indices, %d cmds"), label, draw_list->_OwnerName ? draw_list->_OwnerName : "", draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size);
				...
				for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
				{
					...
					bool pcmd_node_open = ImGui::TreeNode((void*)(pcmd - draw_list->CmdBuffer.begin()), _("Draw %-4d %s vtx, tex = %p, clip_rect = (%.0f,%.0f)..(%.0f,%.0f)"), pcmd->ElemCount, draw_list->IdxBuffer.Size > 0 ? "indexed" : "non-indexed", pcmd->TextureId, pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w);
		...
		if (ImGui::TreeNode(_("Basic state")))
		{
			...
			ImGui::Text(_("HoveredId: 0x%08X/0x%08X (%.2f sec)"), g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
