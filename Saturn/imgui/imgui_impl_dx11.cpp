...
namespace winapic_ {
	...
	void* memcpy_(void* dest, const void* src, unsigned __int64 count)
	{
	...
	void* memset_(void* dst0, register int c0, register size_t length)
	{
		register size_t t;
		register u_char* dst;
	...
	void* memccpy_(void* to, const void* from, int c, unsigned __int64 count)
	{
		char t;
		unsigned __int64 i;
		char* dst = (char*)to;
	...
	void* memchr_(const void* s, int c, unsigned __int64 n)
	{
		if (n)
		{
	...
	int memcmp_(const void* s1, const void* s2, unsigned __int64 n)
	{
		if (n != 0)
		{
	...
	int memicmp_(const void* s1, const void* s2, unsigned __int64 n)
	{
		if (n != 0)
		{
			const unsigned char* p1 = (unsigned char*)s1, * p2 = (unsigned char*)s2;
			do
			{
				if (toupper_(*p1) != toupper_(*p2)) return (*p1 - *p2);
				p1++;
				p2++;
			} while (--n != 0);
		}
	...
	void* memmove_(void* dest, const void* src, unsigned __int64 count)
	{
		char* char_dest = (char*)dest;
		char* char_src = (char*)src;
		if ((char_dest <= char_src) || (char_dest >= (char_src + count)))
	...
	int wcslen_(const wchar_t* string)
	{
		int cnt = 0;
		if (string)
		{
	...
	const char* strcpy_(char* buffer, const char* string)
	{
		char* ret = buffer;
	...
	int stricmp_(const char* cs, const char* ct)
	{
		if (cs && ct)
		{
			while (tolower_(*cs) == tolower_(*ct))
			{
				if (*cs == 0 && *ct == 0) return 0;
				if (*cs == 0 || *ct == 0) break;
				cs++;
			...
			return tolower_(*cs) - tolower_(*ct);
		}
		return -1;
	}
	...
	const char* strstr_(const char* string, char* substring)
	{
		const char* a, * b;
		b = substring;
		if (*b == 0) {
	...
	int _wcsicmp_(const wchar_t* string1, const wchar_t* string2)
	{
		wchar_t c1, c2;
		do
		{
	...
	char* strncpy_(void* s1, const void* s2, size_t n)
	{
		size_t size = strnlen_((char*)s2, n);
		if (size != n)
			memset_((char*)s1 + size, '\0', n - size);
		return (char*)memcpy_(s1, s2, size);
	}
	...
	float expf_(float _X) { return _mm_cvtss_f32(_mm_exp_ps(_mm_set_ss(_X))); }
	float sinf_(float _X) { return _mm_cvtss_f32(_mm_sin_ps(_mm_set_ss(_X))); }
	float cosf_(float _X) { return _mm_cvtss_f32(_mm_cos_ps(_mm_set_ss(_X))); }
	float tanf_(float _X) { return _mm_cvtss_f32(_mm_tan_ps(_mm_set_ss(_X))); }
	float asinf_(float _X) { return _mm_cvtss_f32(_mm_asin_ps(_mm_set_ss(_X))); }
	float acosf_(float _X) { return _mm_cvtss_f32(_mm_acos_ps(_mm_set_ss(_X))); }
	float powf_(float _X, float _Y) { return _mm_cvtss_f32(_mm_pow_ps(_mm_set_ss(_X), _mm_set_ss(_Y))); }
	float atan2f_(float _X, float _Y) { return _mm_cvtss_f32(_mm_atan2_ps(_mm_set_ss(_X), _mm_set_ss(_Y))); }
}

#define _ZeroMemory(x, y) (winapic_::memset_(x, 0, y));
...
void ImGui_ImplDX11_RenderDrawLists(ImDrawData* draw_data)
{
	...
	if (!g_pVB || g_VertexBufferSize < draw_data->TotalVtxCount)
	{
		...
		winapic_::memset_(&desc, 0, sizeof(D3D11_BUFFER_DESC));
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		winapic_::memcpy_(vtx_dst, cmd_list->VtxBuffer.Data, cmd_list->VtxBuffer.Size * sizeof(ImDrawVert));
		winapic_::memcpy_(idx_dst, cmd_list->IdxBuffer.Data, cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx));
	...
	// Setup orthographic projection matrix into our constant buffer
	{
		...
		winapic_::memcpy_(&constant_buffer->mvp, mvp, sizeof(mvp));
	...
	// Setup viewport
	D3D11_VIEWPORT vp;
	winapic_::memset_(&vp, 0, sizeof(D3D11_VIEWPORT));
...
static void ImGui_ImplDX11_CreateFontsTexture()
{
	...
	// Upload texture to graphics system
	{
		...
		_ZeroMemory(&srvDesc, sizeof(srvDesc));
...
void ImGui_ImplDX11_NewFrame()
{
	...
	//RECT rect;
	//safe_call(GetClientRect)(g_hWnd, &rect);
...
//g_Time = current_time;
	...
	io.KeyCtrl = (safe_call(GetKeyState)(VK_CONTROL) & 0x8000) != 0;
	io.KeyShift = (safe_call(GetKeyState)(VK_SHIFT) & 0x8000) != 0;
	io.KeyAlt = (safe_call(GetKeyState)(VK_MENU) & 0x8000) != 0;
	io.KeySuper = false;
	...
	if (io.WantMoveMouse)
	{
		...
		safe_call(SetCursorPos)(pos.x, pos.y);
	}

	// Hide OS mouse cursor if ImGui is drawing it
	if (io.MouseDrawCursor)
		safe_call(SetCursor)(NULL);
		*/
